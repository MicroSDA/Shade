...
void se::Log::Print(const std::string& message, const LogLevel& level)
{
	GetInstance()._Print(message, level);
}

void se::Log::_Print(const std::string& message, const LogLevel& level)
{
	switch (level)
	{
	case LogLevel::INFO_SECONDARY:
		printf("\033[1;30m[%s] %s\n", CurrentDateTime().c_str(), message.c_str()); // grey
		break;
	case LogLevel::INFO_PRIMARY:
